...
parser.add_argument("--model", type=str, default="gru", choices=["gru", "lstm", "behavenet"],
...
sensor_df[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']] = scaler.fit_transform(
    sensor_df[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']]
)

# Augmentation function for sensor data
...
sensor_df[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']] = augment_sensor_data(
    sensor_df[['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z']].values
)
...
sensor_df['accel_magnitude'] = np.sqrt(sensor_df['accel_x']**2 + sensor_df['accel_y']**2 + sensor_df['accel_z']**2)
sensor_df['gyro_magnitude'] = np.sqrt(sensor_df['gyro_x']**2 + sensor_df['gyro_y']**2 + sensor_df['gyro_z']**2)

# Collect sensor windows
...
for i in range(len(displacement_vectors)):
    ...
    if end_idx <= len(sensor_df):
        sensor_window = sensor_df.iloc[start_idx:end_idx][['accel_x', 'accel_y', 'accel_z', 'gyro_x', 'gyro_y', 'gyro_z', 'accel_magnitude', 'gyro_magnitude']].values
        sensor_sequences.append(sensor_window)

sensor_sequences = np.array(sensor_sequences)
displacement_features = np.repeat(np.array(displacement_vectors).reshape(-1, 1, 2), window_size, axis=1)
...
y = torch.tensor([key_to_label(key) for key in keys_df['key'][:len(displacement_vectors)]], dtype=torch.long)

dataset = TensorDataset(X, y)

# Function to get the model based on the specified type
def get_model(model_type, input_dim, hidden_dim, output_dim, num_layers, dropout_rate):
    if model_type == "gru":
        return AttentionGRUModel(input_dim, hidden_dim, output_dim, num_layers, dropout_rate)
    elif model_type == "lstm":
        return AttentionLSTMModel(input_dim, hidden_dim, output_dim, num_layers, dropout_rate)
    elif model_type == "behavenet":
        return BehaviorNetModel(input_dim, hidden_dim, output_dim, num_layers, dropout_rate)
    else:
...
def objective(trial):
    ...
    for fold, (train_idx, val_idx) in enumerate(kfold.split(X, y)):
        ...
        for epoch in range(NUM_EPOCHS):
            ...
            print(f"Trial {trial.number + 1}/{NUM_TRIALS}, Epoch {epoch + 1}/{NUM_EPOCHS} | "
                  f"Train Loss: {avg_loss:.4f} | Train Accuracy: {train_accuracy:.3f} | "
                  f"Val Loss: {avg_val_loss:.4f} | Val Accuracy: {val_accuracy:.3f}", end="\r")

            # Check for potential overfitting
            if (train_accuracy - val_accuracy) > OVERFITTING_THRESHOLD:
                print()
            ...
            # Update best accuracy and check for early stopping
    ...
    avg_accuracy /= 5  # Average over 5 folds

    if avg_accuracy > best_trial_accuracy:
        best_trial_accuracy = avg_accuracy
        torch.save(best_model_state, f"./data/models/{args.model}_best_model.pth")
        print(f"New best model saved with validation accuracy: {avg_accuracy:.3f}")
    else:
        print()
    return avg_accuracy
