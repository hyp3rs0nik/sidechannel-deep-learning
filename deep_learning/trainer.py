...
def get_optimized_dataloader(dataset, batch_size, shuffle, num_workers=4):
    return DataLoader(
        ...
        shuffle=shuffle,
        pin_memory=True,
        num_workers=num_workers,  # Reduced number of workers
...
def objective(trial, config, X, y, num_classes, model_type, k_folds=5, device="cuda"):
    ...
    for fold, (train_idx, val_idx) in enumerate(skf.split(X_cpu, y_cpu), 1):
        ...
        y_val = y[val_idx]
        model = RNNModel(
            model_type, X_train.shape[2], hidden_size, dropout_rate, num_classes
        ).to(device)
        ...
        for epoch in range(1, epochs + 1):
            try:
                ...
                val_loss, val_acc = validate_fold(
                    model, criterion, scaler_obj, val_loader, device, scheduler
                )
                print_progress(
                    trial.number,
                    fold,
...
def main():
    ...
    for epoch in range(1, config["epochs"] + 1):
        try:
            ...
            val_loss, val_acc = validate_fold(
                model, criterion, scaler_final, train_loader, device, scheduler
            )
            print_progress(
                "Final",
                "Final",
            ...
            break
    print()
    model_save_path = os.path.join("./data/models/deep_learning/", f"{args.model}.pt")
